...
from models import Student, Teacher, User, Token
from auth import create_database_token, generate_token, get_current_user, get_password_hash, token_expiry
from passlib.context import CryptContext
from db_setup import get_db
import crud
import uvicorn
from schemas import Class_LevelBase, Class_LevelCreate, Class_LevelOut, GradeCreate, GradeOut, BetygOutStudent, GradeUpdate, FiluppladdningCreate, FiluppladdningOut, HomeworkCreate, HomeworkInUpdate, MeddelandeCreate, MeddelandeOut, MembershipUpdate, RecommendedResourceCreate, RecommendedResourceOut, RoleCreate, StudentCreate, StudentOut, SubjectCreate, SubjectOut, SubjectUpdate, TeacherCreate, TeacherOut, UserInUpdate
from schemas import GetUser, HomeworkBase, HomeworkOut, UserIn, UserOut, MembershipOut
from models import Class_Level, Grade, File_Attachment, Homework, Message, Recommended_Resource, Role, Subject, User
...
def add_teacher(
    ...
    """
    # Check if a teacher profile already exists for the user
    ...
    new_teacher = Teacher(
        ...
        photo=teacher_params.photo,  # Include photo if provided
    )
    db.add(new_teacher)
    db.commit()
    ...
    return [
        {
            ...
            "photo": teacher.photo,  # Include photo if available
        }
...
def get_all_students(
    ...
    """
    Retrieve all students from the database, including related data.
    """
    students = db.query(Student).options(
        joinedload(Student.user),
        joinedload(Student.arskurs),
...
def add_student(
    ...
    existing_student = db.query(Student).filter(Student.user_id == student_params.user_id).first()
    if existing_student:
        raise HTTPException(status_code=400, detail="Student profile already exists for this user")

    # Create a new student entry
    new_student = Student(
        user_id=student_params.user_id,
        arskurs_id=student_params.arskurs_id,
        school_id=student_params.school_id,
        teacher_id=student_params.teacher_id,  # Include teacher_id
        enrollment_date=student_params.enrollment_date,
    )
    db.add(new_student)
    db.commit()
...
def create_homework_view(
    ...
    if homework.user_id:
        ...
        arskurs = database.query(Class_Level).filter(Class_Level.id == homework.arskurs_id).first()
        if not arskurs:
